...
import { documentsApi, DOCUMENT_CATEGORIES, REQUIRED_DOCUMENTS, TOTAL_REQUIRED_DOCUMENTS, type DocumentRow, } from '../../../3-data-tier/api/documentsApi';
...
export function DocumentChecklistModal({
  ...
  const [ setDobSuggestion] = useState<{ dateOfBirth: string; source: string } | null>(null);
  ...
  mutationFn: async ({ type, file }: { type: DocumentType; file: File }) => {
    ...
    // Silent on success — the coach only ever sees something if it's blocked.
if (aiResult.data?.dateOfBirth) {
  ...
  return (
    ...
    >
      {/* Wider modal, no horizontal scrolling needed anywhere inside it */}
      <div
      ...
      >
        ...
        <div className="flex justify-between items-start p-6 pb-4 border-b border-slate-100">
          ...
          <button onClick={onClose} className="text-slate-400 hover:text-slate-600 active:scale-90 shrink-0 transition-[color,transform]">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="px-6 pt-4">
          ...
          <div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden">
            <div
              className={`h-full rounded-full transition-[width] motion-reduce:transition-none ${isComplete ? 'bg-green-500' : 'bg-blue-600'}`}
  ...
  <div className="mx-6 mt-4 p-3 bg-red-50 border border-red-300 rounded-lg">
    <p className="text-xs text-red-700 font-medium">
      This athlete would be {ageWarning.age} years old as of {ageWarning.asOfDate} — over the age limit (25 and under) for ILOPRISAA eligibility. This date of birth was NOT saved.
    </p>
    <button
      type="button"
              ...
              return (
                <div key={category.id} className="border border-slate-200 rounded-xl overflow-hidden">
                  ...
                  >
                    ...
                    <ChevronDown className={`w-4 h-4 text-slate-400 shrink-0 transition-transform motion-reduce:transition-none ${isOpenCat ? 'rotate-180' : ''}`} />
                  </button>
